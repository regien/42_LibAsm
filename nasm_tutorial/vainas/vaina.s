; fib.s
; $ mean <- address here  - format.pendejada (trick to get the len)

global  _main
extern  _printf
section .data
format:
	.pendejada db  `%20ld`,  10, 0
...
;	mov		rbp, rsp
;	sub		rsp, 8
;    mov     rdi, [rel format.pendejada]         ; set 1st parameter (format)
    lea     rdi, [rel format.pendejada]         ; set 1st parameter (format)
    ;mov     rdi, format.pendejada         ; set 1st parameter (format)
